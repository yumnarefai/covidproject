...
def get_organized_data(data: list) -> list[Day]:
    ...
    for info in data[5:]:
        ...
        t = sum([pollutants[p].total for p in pollutants])
...
def get_total_per_source(dataset: list[Day]) -> dict[str, tuple[float, float]]:
    ...
    for source in main_sources:
        total_2019, total_2020 = helper_source(dataset, source)
        data[source] = (total_2019, total_2020)
    return data


def helper_source(dataset: list[Day], source: str) -> tuple[float, float]:
    """ a helper function for get_total_per_source
    Returns a tuple co-responding to total pollutants emission for a given source
    in 2019 (before Covid) and 2020 (during Covid).
    """
    total_2019 = 0
    total_2020 = 0
    for day in dataset:
...
def get_total_per_industry(dataset: list[Day]) -> dict[str, tuple[float, float]]:
    ...
    for industry in all_industries:
        total_2019, total_2020 = helper_industry(dataset, all_industries, industry)
        data[industry] = (total_2019, total_2020)
    return data


def helper_industry(dataset: list[Day], all_industries: dict, industry: str) -> tuple[float, float]:
    """ a helper function for get_total_per_industry
    Returns a tuple co-responding to total pollutants emission for a given industry
    in 2019 (before Covid) and 2020 (during Covid).
    """
    total_2019 = 0
    total_2020 = 0
